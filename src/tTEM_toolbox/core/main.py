...
class ProcessTTEM():
    def __init__(self, ttem_path, **kwargs):
        ...
        self.kwargs = kwargs
        ttem = tc.process_ttem.format_ttem(fname=self.ttem_path,
                                                 **self.kwargs)
        self.ttem = ttem
    def data(self):
        return self.ttem
    def fill(self, factor=100):
        ttem_fill = tc.process_ttem.format_ttem(fname=self.ttem, filling=True, factor=factor)
        return ttem_fill
    def ttem_well_connect(self, distance=500, wellupscale=10, check_corr=np.nan, debug=False):
        if np.isin("welllog", list(self.kwargs.keys())):
            if isinstance(self.kwargs['welllog'], str):
                welllog_upscale = tc.process_well.format_well(self.kwargs["welllog"], upscale=wellupscale)
            elif isinstance(self.kwargs['welllog'], pd.DataFrame):
                welllog_upscale = self.kwargs['welllog']
            if ~np.isnan(check_corr):
                ttem_match, well_match = tc.bootstrap.select_closest(self.ttem, welllog_upscale,
                                                                     distance=distance, showskip=False)
                welllog_upscale = tc.bootstrap.corr_well_filter(ttem_match, well_match, corr_thershold=check_corr)
                pre_bootstrap, Resistivity, Thickness_ratio,matched_ttem, matched_well = tc.bootstrap.pre_bootstrap(self.ttem, welllog_upscale,
                                                                                     distance=distance)
            else:
                pre_bootstrap, Resistivity, Thickness_ratio,matched_ttem, matched_well = tc.bootstrap.pre_bootstrap(self.ttem, welllog_upscale,
                                                                                     distance=distance)
            fine, mix, coarse = tc.bootstrap.bootstrap(Resistivity, Thickness_ratio)
            pack_bootstrap_result = pd.DataFrame({'fine':fine,'mix':mix,'coarse':coarse})
            Resi_conf_df = tc.bootstrap.packup(fine, mix, coarse)
            rk_trans=tc.Rock_trans.rock_transform(self.ttem, Resi_conf_df)
        else:
            raise ValueError("welllog not found!")
        if debug is True:
            return pre_bootstrap,rk_trans, pack_bootstrap_result, Resi_conf_df, matched_ttem, matched_well
        else:
...
class ProcessGamma():
    ...
    def process(self):
        if np.isin('columns', list(self.kwargs.keys())):
            ori = tc.process_gamma.load_gamma(self.gamma, columns=self.kwargs["columns"])
        else:
            ori = tc.process_gamma.load_gamma(self.gamma)
        ori = tc.process_gamma.georeference(ori, self.gammaloc)
        if np.isin("rolling", list(self.kwargs.keys())):
            if self.kwargs["rolling"] is True:
                try:
                    ori = tc.process_gamma.rolling_average(ori,
    ...
    def gr_wlg_combine(self,ori):
        try:
            gr_wlg = tc.process_gamma.gamma_well_connect(ori, self.kwargs["welllog"])
        except:
    ...
    def gr_ttem_combine(self, ori, **kwargs):
        ...
        gr_ttem = tc.process_gamma.gamma_ttem_connect(ori_result, ttem_result)
        return gr_ttem
class GWSurface():
    """
    Parameters
    ...
    def __init__(self, waterwell,
        ...
        if not self.well.isdigit():
            ...
            try:
                ...
                try:
                    self.well = pd.read_csv(self.well, header=1)
                    self.well_list = self.well['SiteNumber'].values
                except:
                    raise ValueError('Not able to determine the file format')
        else:
            self.well_list = self.well
        self.ds = xr.Dataset()
        for i in self.well_list:
            tmp_ds = tc.process_well.format_usgs_water(str(i), self.elevation_type, **self.kwargs)
    ...
    def format(self, elevation=None, time='2022-03'):
        ...
        if isinstance(time, str):
            self.df = tc.process_well.water_head_format(self.ds, time=str(time), header=header, elevation=elevation)
        else:
            self.df = tc.process_well.water_head_format(self.ds, time=str(time[0]), header=header, elevation=elevation)
            for i in time[1:]:
                tmp_df = tc.process_well.water_head_format(self.ds, time=str(i), header=header, elevation=elevation)
